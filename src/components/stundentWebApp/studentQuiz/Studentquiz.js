...
const Studentquiz = ({ takequiz, setSelectedQuiz, setCompletedQuiz}) => {
    const [currentPage, setCurrentPage] = useState('Activequiz');

    const renderContent = () => {
        switch (currentPage) {
            case 'Activequiz':
                return <StudentActivequiz takequiz={takequiz} setSelectedQuiz={setSelectedQuiz}  />;
            case 'Completequiz':
                return <StudentCompletequiz takequiz={takequiz}  setCompletedQuiz={setCompletedQuiz}/>;
            case 'InCompletequiz':
